...
import {
  visitorsSchema,
  VisitorsSchemaFormDataType,
} from "@/constants/validations/register-visitor";
...
export default function InviteVisitor() {
  ...
  const handleSubmit = async (data: VisitorsSchemaFormDataType) => {
    ...
    const fullPhoneNumber =
      selectedCountryCode.dialCode.replace("+", "") + cleanPhoneNumber;
        ...
        <CardContent className="space-y-4">
          <div className="mb-6">
            ...
            <p className="text-gray-600">
              Create a temporary tag pass for a visitor
            </p>
          </div>
          <div
            className={"border border-[#5283EB] bg-[#5283EB]/20 rounded-sm p-2"}
          >
            <h1 className={"text-[#063776] font-semibold font-roboto"}>
              Visitor Tag Number: BH-0007
            </h1>
            <p className={"text-[#00499A]"}>
              This unique identifier will be used to track the visitor in the
              system
            </p>
          </div>
          <Form {...form}>
